...
if __name__ == "__main__":
    ...
    if args.is_4bit==False: # 一般还是推荐int8模式.
        config = PeftConfig.from_pretrained(args.model_name_or_path)
        tokenizer = AutoTokenizer.from_pretrained(config.base_model_name_or_path,use_fast=False)
        tokenizer.pad_token = tokenizer.eos_token # 默认是缺少pad 所以设置为eos
        model = AutoModelForCausalLM.from_pretrained(config.base_model_name_or_path,device_map='auto',torch_dtype=torch.float16,load_in_8bit=True,low_cpu_mem_usage=True)
        model = PeftModel.from_pretrained(model, args.model_name_or_path, device_map={"": 0})
        model.eval()
    else:
        from auto_gptq import AutoGPTQForCausalLM # int4量化的库包: auto_gptq
    ...
    if torch.__version__ >= "2" and sys.platform != "win32":
        model = torch.compile(model) # 对模型进行加速.
